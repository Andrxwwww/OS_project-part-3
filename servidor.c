...
DadosServidor *db = NULL;               // Variável que vai ficar com um pointer para  memória partilhada
 ...
 *              preenchendo so_errorem todos os elementos o campo idProduto=PRODUCT_NOT_FOUND (“Limpa” a
 ...
 *              so_error e retorna ershutdownAndExit_S7()ro (vai para S7). Caso contrário, dá so_success.
...
int initShm_S1() {
    ...
    shmId = shmGet(); // dá open de uma shared memory com uma IPC já existente
    if (shmId != RET_ERROR){ // se ao dar open der sucesso
        db = shmAttach(shmId); // liga a variável db a essa SHM
        if (db == NULL){
            so_error("S1.1", "");
        } else {
    ...
    // S1.2
    if (errno != ENOENT){ // verifica se a variavel errno é diferente de ENOENT(se um file ou dir não existe)
        so_error("S1.2", "");
        return RET_ERROR;
    } else {
        so_success("S1.2", "");
    ...
    //1.3
    shmId = shmCreate(1);// cria uma shared memory com o tamanho de 1 (para conter as 2 listas)
    if (shmId != RET_ERROR){
        db = shmAttach(shmId);// liga a variável db a essa SHM
        if (db == NULL){
            so_error("S1.3", "db nao encontrado");
            return RET_ERROR;
        }else{
            result = RET_SUCCESS;
    ...
    //1.4
    for (int i = 0; i < MAX_USERS; i++){ // corre a lista de utilizadores
        db->listUsers[i].nif = USER_NOT_FOUND;
    }
    for (int i = 0; i < MAX_PRODUCTS; i++){ // corre a lista de produtos
    ...
    //1.5
    FILE *fpUsers = fopen(FILE_DATABASE_USERS, "r"); // abre o ficheiro FILE_DATABASE_USERS em read mode
    if(fpUsers == NULL){
        so_error("S1.5","");
        return RET_ERROR;
    }
    for(int i = 0; i<MAX_USERS; ++i){ // corre a lista de utilizadores
        fread(&db->listUsers[i], sizeof(Login), 1, fpUsers); // le o ficheiro lista de utilizadores na memória partilhada com a informação dos utilizadores
        db->listUsers[i].pidCliente = -1; // preenche sempre os campos pidCliente e pidServidorDedicado com o valor -1
    ...
    //1.6
    FILE* fpProducts = fopen(FILE_DATABASE_PRODUCTS, "r");
    if(fpProducts == NULL){
        so_error("S1.6","");
        return RET_ERROR;
    }
    fread(&db->listProducts, sizeof(Produto), MAX_PRODUCTS, fpProducts); // le o ficheiro lista de produtos na memória partilhada com a informação dos produtos
...
int initMsg_S2() {
    ...
    //2.1
    msgId = msgGet(); // dá open de uma message queue com uma IPC já existente
    if (msgId != RET_ERROR){
        if (msgRemove(msgId) == RET_ERROR){ // se ao remover a message queue der erro
            so_error("S2.1", "");
            return RET_ERROR;
        } else {
            so_success("S2.1", "");
    ...
    //2.2
    msgId = msgCreate(); // cria uma message queue com a KEY IPC_KEY
    if (msgId == RET_ERROR){
        so_error("S2.2", "");
        return RET_ERROR;
    } else {
...
int initSem_S3() {
    ...
    //3.2
    semId = semCreate(3); // cria 3 semáforos mais precisamente SEM_USERS , SEM_PRODUCTS e SEM_NR_SRV_DEDICADOS
    if (semId == RET_ERROR){
        so_error("S3.2", "");
        return RET_ERROR;
    } else {
    ...
    //3.3 SEM_USERS e SEM_NR_SRV_DEDICADOS nao sao usados v1.3

    if (semNrSetValue(semId, SEM_USERS, SEM_MUTEX_INITIAL_VALUE) == RET_ERROR){ // metemos o mutex inicial value por causa da parte de trabalhar com "exclusão mútua"
        so_error("S3.3", "");
        return RET_ERROR;
    } else if(semNrSetValue(semId, SEM_PRODUCTS, SEM_MUTEX_INITIAL_VALUE) == RET_ERROR){
        so_error("S3.3", "");
        return RET_ERROR;
    } else if(semNrSetValue(semId, SEM_NR_SRV_DEDICADOS, 0 ) == RET_ERROR){
        so_error("S3.3", "");
        return RET_ERROR;
    } else {
        so_success("S3.3", "");
...
int triggerSignals_S4() {
    ...
    if (signal(SIGINT, handleSignalSIGINT_S8) == SIG_ERR) {
        so_error("S4", "Erro no SIGINT");
    } else if (signal(SIGCHLD, handleSignalSIGCHLD_S9) == SIG_ERR) {
        so_error("S4", "Erro no sinal SIGCHLD");
    } else {
        result = RET_SUCCESS;
        so_success("S4","");
    }
...
MsgContent receiveClientLogin_S5() {
    ...
    //S5
    if (msgReceive(msgId, &msg, MSGTYPE_LOGIN) == RET_ERROR){ // le da message queue
        so_error("S5", "message not read correctly");
        return msg;
    } else {
        so_success("S5", "%d %s %d", msg.msgData.infoLogin.nif, msg.msgData.infoLogin.senha, msg.msgData.infoLogin.pidCliente);
    }

    so_debug("> [@return nif:%d, senha:%s, pidCliente:%d]", msg.msgData.infoLogin.nif, msg.msgData.infoLogin.senha, msg.msgData.infoLogin.pidCliente);
    return msg;
}
 ...
 *              Dedicado: PID <pidServidorDedicado>", incrementa a variável nrServidoresDedicados,
...
int createServidorDedicado_S6() {
    ...
    //S6
    pid_filho = fork();
    if (pid_filho == RET_ERROR){
        so_error("S6", "error in fork");
        return RET_ERROR;
    } else if (pid_filho == PROCESSO_FILHO){
        triggerSignals_SD10();
    } else {
...
void shutdownAndExit_S7() {
    ...
    // S7.2  Percorre a lista de utilizadores. 
    for (int i = 0; i < MAX_USERS; i++){ 
    ...
    // S7.4  Reescreve o ficheiro bd_produtos.dat, mas incluindo apenas os produtos existentes
    FILE *fpProducts = fopen(FILE_DATABASE_PRODUCTS, "w");
    if(fpProducts == NULL){
        so_error("S7.4","");
    }else{ 
        for(int i = 0; i<MAX_PRODUCTS; i++){
            if (db->listProducts[i].stock > 0) {
                fwrite(&(db->listProducts[i]),sizeof(Produto),1,fpProducts); // escreve no ficheiro os produtos com stock > 0
            }
        }
        fclose(fpProducts);
    }
    ...
    // S7.5  Apaga todos os elementos IPC (SHM, SEM e MSG) que tenham sido criados pelo Servidor
    if (shmId != RET_ERROR){
        if (shmRemove(shmId) == RET_ERROR){
            so_error("S7.5", "");
        } else if (semRemove(semId) == RET_ERROR){
            so_error("S7.5", "");
        } else if (msgRemove(msgId) == RET_ERROR){
            so_error("S7.5", "");
        } else {
            so_success("S7.5", "");
        }
...
void handleSignalSIGCHLD_S9(int sinalRecebido) {
    ...
    wait(NULL);
    so_success("S9", "Terminou Servidor Dedicado %d", msg.msgData.infoLogin.pidServidorDedicado);
...
int triggerSignals_SD10() {
    ...
    if (signal(SIGUSR1, handleSignalSIGUSR1_SD18) == SIG_ERR) {
        so_error("SD10", "Erro no SIGUSR1");
    } else if (signal(SIGINT, SIG_IGN) == SIG_ERR) {
        so_error("SD10", "Erro no sinal SIGINT");
    } else {
        result = RET_SUCCESS;
        so_success("SD10","");
    }
...
int validateRequest_SD11(Login request) {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("< [@param request.nif:%d, request.senha:%s, request.pidCliente:%d]", request.nif, request.senha, request.pidCliente);

   if (request.pidCliente > RET_ERROR){
        so_success("SD11", "pedido de login com sucesso");
        result = RET_SUCCESS;
    } else {
        so_error("SD11", "pedido de login com sucesso");
    }
...
int searchUserDB_SD12(Login request) {
    ...
    //SD12
    int lock = 0; // Variável para verificar se o nif está correto
    for (int i = 0; i < MAX_USERS; i++){
        //SD12.1
        if ( db->listUsers[i].nif == request.nif){
            if (strncmp(db->listUsers[i].senha, request.senha, sizeof(request.senha)) == 0){ // Compara as senhas usando strncmp
    ...
    if (lock == 0){
        so_error("SD12.1", "nif errado");
    }
    //SD12.2 - Cria uma resposta indicando erro ao Cliente, preenchendo na estrutura Login o campo pidServidorDedicado=-1.
    msg.msgData.infoLogin.pidServidorDedicado = -1;
    if (msgSend(msgId, msg, msg.msgData.infoLogin.pidCliente) == RET_ERROR){
        so_error("SD12.2", "erro ao enviar mensagem");
    } else {
        so_success("SD12.2", "mensagem enviada");
    }
...
int reserveUserDB_SD13(int indexClient, int pidCliente) {
    ...
    //SD13 reserva o utilizador mudando na db o pidServidorDedicado e o pidCliente
    if(indexClient>=0 && indexClient< MAX_USERS){
        db->listUsers[indexClient].pidServidorDedicado = getpid();
        db->listUsers[indexClient].pidCliente = pidCliente;
        so_success("SD13", " utilizador reservado com sucesspidClienteo");
        result = RET_SUCCESS;
    }else{
        so_error("SD13", "indexClient invalido");
    }
...
int sendProductList_SD14(int indexClient, int pidCliente) {
    ...
    //14.1 mete a informacao do utilizador da db na mensagem
    ...
    //14.2 vai mandar a lista de produtos ao cliente 
    for (int i = 0; i < MAX_PRODUCTS; i++){
        if (db->listProducts[i].stock > 0){
            msg.msgData.infoProduto = db->listProducts[i];
            if (msgSend(msgId, msg, pidCliente) == RET_ERROR){
                so_error("SD14.2", "erro ao enviar mensagem");
                return RET_ERROR;
            }
        }
    }
    so_success("SD14.2", "mensagem enviada");

    //14.3 envia uma mensagem final com o idProduto = FIM_LISTA_PRODUTOS (ou seja nao ha mais produtos para listar)
    msg.msgData.infoProduto.idProduto = FIM_LISTA_PRODUTOS;
    if (msgSend(msgId, msg, pidCliente) == RET_ERROR){
        so_error("SD14.3", "erro ao enviar mensagem");
        return RET_ERROR;
    } else {
        so_success("SD14.3", "mensagem enviada");
        result = RET_SUCCESS;
    }
...
MsgContent receiveClientOrder_SD15() {
    ...
    if (msgReceive(msgId, &msg, getpid() ) == RET_ERROR){
        so_error("SD15", "message not read correctly");
        return msg;
    } else {
        so_success("SD15", "%d %d %d", msg.msgData.infoLogin.nif, msg.msgData.infoProduto.idProduto, msg.msgData.infoLogin.pidCliente);
    }

    so_debug("> [@return nif:%d, idProduto:%d, pidCliente:%d]", msg.msgData.infoLogin.nif, msg.msgData.infoProduto.idProduto, msg.msgData.infoLogin.pidCliente);
    return msg;
}
...
int sendPurchaseAck_SD16(int idProduto, int pidCliente) {
    ...
    int lock = 0; //variavel para controlar se o produto foi comprado ou nao/nao encontrado (0 - nao, 1 - sim)

    //SD16.1 se o idProduto enviado pelo cliente for dif do id do produto do PRODUCT_NOT_FOUND entao da erro
    if (idProduto == PRODUCT_NOT_FOUND){ 
        msg.msgData.infoProduto.idProduto = PRODUTO_NAO_COMPRADO;
        so_error("SD16.1", "produto cancelado");
    } else {

        //SD16.2 percorre a lista de produtos, procurando pelo produto (verificando o stock) com o idProduto recebido no pedido do Cliente.
        for (int i = 0; i < MAX_PRODUCTS; i++){
            if (db->listProducts[i].idProduto == idProduto){
                if (db->listProducts[i].stock > 0){
                    msg.msgData.infoProduto.idProduto = PRODUTO_COMPRADO;
                    semNrAddValue(semId, SEM_PRODUCTS, -1); // desbloqueia o semaforo dos produtos para decrementar o stock
                    db->listProducts[i].stock--;
                    so_success("SD16.2", "produto comprado"); 
                    semNrAddValue(semId, SEM_PRODUCTS, 1); // bloqueia o semaforo dos produtos
                    lock = 1;
                } else {
                    lock = 1;
                    msg.msgData.infoProduto.idProduto = PRODUTO_NAO_COMPRADO;
                    so_error("SD16.2", "produto nao comprado / stock esgotado");
                }
            }
        }
        if (lock == 0){
            msg.msgData.infoProduto.idProduto = PRODUTO_NAO_COMPRADO;
            so_error("SD16.2", "produto nao comprado / nao encontrado");
        }
    }

    //SD16.3 Envia, usando como msgType o PID do processo Cliente, a mensagem de resposta de conclusão ao Cliente
    if (msgSend(msgId, msg , msg.msgData.infoLogin.pidCliente ) == RET_ERROR){
        so_error("SD16.3", "erro ao enviar mensagem");
    } else {
        so_success("SD16.3", "mensagem enviada");
        result = RET_SUCCESS;
    }
...
void shutdownAndExit_SD17() {
    ...
    // SD17.1 mete nos campos pidServidorDedicado=-1 e pidCliente=-1
    ...
    so_success("SD17.1", "atualizado na lista de utilizadores");
...
void handleSignalSIGUSR1_SD18(int sinalRecebido) {
    ...
    // SD18.1 Envia um sinal SIGUSR2 ao Cliente (para avisá-lo do Shutdown)
    kill(msg.msgData.infoLogin.pidCliente, SIGUSR2);
    so_success("SD18", "enviado sinal SIGUSR2 ao cliente");
    shutdownAndExit_SD17();
