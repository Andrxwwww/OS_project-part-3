...
int initMsg_C1() {
    ...
    //C1 abre a message queue
    if (msgGet() == RET_ERROR) {
        so_error("C1","Não foi possível abrir a Message Queue");
        return RET_ERROR;
    } else {
...
int triggerSignals_C2() {
    ...
    //C2 arma e trata os sinais SIGUSR2, SIGINT e SIGALRM
    if (signal(SIGUSR2, trataSinalSIGUSR2_C10) == SIG_ERR) {
        so_error("C2", "Erro no SIGUSR2");
    } else if (signal(SIGINT, trataSinalSIGINT_C11) == SIG_ERR) {
        so_error("C2", "Erro no sinal SIGINT");
    } else if (signal(SIGALRM, trataSinalSIGALRM_C12) == SIG_ERR) {
...
MsgContent getDadosPedidoUtilizador_C3() {
    ...
    so_debug("<");
    //C3
    pid_t pidCliente = getpid();
    printf("Insira o seu NIF: "); //pede para o utilizador introduzir o NIF
    ...
    printf("Insira a sua senha: "); //pede para o utilizador introduzir a senha
    if (scanf("%s", msg.msgData.infoLogin.senha) != 1) { // verifica se a leitura do STDIN foi bem sucedida
        so_error("C3", "Erro ao ler a senha");
        msg.msgData.infoLogin.nif = USER_NOT_FOUND;
        exit(RET_ERROR);
    } else {
        msg.msgData.infoLogin.pidCliente = pidCliente; // preenche o campo pidCliente com o PID do seu próprio processo Cliente
        so_success("C3", "%d %s %d\n", msg.msgData.infoLogin.nif, msg.msgData.infoLogin.senha, msg.msgData.infoLogin.pidCliente);
    }
...
int sendClientLogin_C4(MsgContent msg) {
    ...
    //C4
    // envia a mensagem para a Message Queue
    if (msgSend(msgId, msg, MSGTYPE_LOGIN) == RET_ERROR) {
        so_error("C4", "Erro ao enviar a mensagem para a Message Queue");
        return RET_ERROR;
    } else {
        so_success("C4", "Mensagem enviada com sucesso");
        result = RET_SUCCESS;
    }
...
void configuraTemporizador_C5(int tempoEspera) {
    ...
    //C5 configura um alarme com o valor de tempoEspera segundos
    alarm(tempoEspera);
...
int receiveProductList_C6() {
    ...
    //C6 abre a message queue
    if (msgReceive(msgId, &msg, getpid()) == RET_ERROR) {
        so_error("C6.2","Não foi possível abrir a Message Queue");
        return RET_ERROR;
    }else {
        so_success("C6","%s %d %d", msg.msgData.infoLogin.nome, msg.msgData.infoLogin.saldo, msg.msgData.infoLogin.pidServidorDedicado);
    }

    // C6.1 desativa o alarme
    alarm(0);

    // C6.2 mete o pidServidorDedicado a -1
    if (msg.msgData.infoLogin.pidServidorDedicado == -1) {
        so_error("C6.2", "Erro na autenticação do Servidor Dedicado");
        return RET_ERROR;
    } else {
        printf("Lista de Produtos Disponíveis:\n");
    }

    // C6.3 - C6.5  extrai da mensagem recebida o Produto especificado até ao fim da lista de produtos e depois verifica se recebeu a mensagem com o pid do cliente 
    while (msg.msgData.infoProduto.idProduto != FIM_LISTA_PRODUTOS){
        printf("%d %s %s %d", msg.msgData.infoProduto.idProduto, msg.msgData.infoProduto.nomeProduto, msg.msgData.infoProduto.categoria, msg.msgData.infoProduto.preco);
        if (msgReceive(msgId, &msg, getpid()) == RET_ERROR) {
            so_error("C6.5", "Erro ao receber a Message Queue");
            return RET_ERROR;
        }
    }
    so_success("C6.3", "");
...
int getIdProdutoUtilizador_C7() {
    ...
    //C7
    int idProduto;
    printf("Insira o id do produto que deseja adquirir: ");
    if (scanf("%d", &idProduto) != RET_ERROR) {
        if (idProduto <= 0) {
            so_error("C7", "Erro ao ler o id do produto");
...
int sendClientOrder_C8(int idProduto, int pidServidorDedicado) {
    ...
    if (msgSend(msgId, msg, pidServidorDedicado)  == RET_ERROR) {
        so_error("C8", "Erro ao enviar a mensagem para a Message Queue");
        return RET_ERROR;
    } else {
        so_success("C8", "Mensagem enviada com sucesso");
        result = RET_SUCCESS;
    }
...
void receivePurchaseAck_C9() {
    ...
    //C9 - recebe a mensagem do servidor dedicado
    if (msgReceive(msgId, &msg, getpid() ) == RET_ERROR) {
        so_error("C9", "Erro ao receber a Message Queue");
    } else {
...
void trataSinalSIGUSR2_C10(int sinalRecebido) {
    ...
    if(sinalRecebido == SIGUSR2){
        so_success("C10", "Shutdown Servidor Dedicado");
        exit(RET_SUCCESS);
    }
